...
def gameLoop():
    ...
    agents.extend([RiskTolerant(stockMarket.getStockListings()) for _ in range(x)])  # makes x of tolerant type of agent
    agents.extend([RiskAvers(stockMarket.getStockListings()) for _ in range(x)])  # makes x of avers type of agent
